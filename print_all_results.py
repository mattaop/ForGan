...
def print_results(folder):
    ...
    for sub_folder in sub_folders:
        try:
            ...
            print(cfg['layers'] + "  |    %d    |     %d     |     %d     |    %d    |    %d   | %.3f |%.2f| %.2f| %.2f| "
                  % ( cfg['discriminator_epochs'], cfg['noise_vector_size'], cfg['batch_size'], cfg['generator_nodes'], cfg['discriminator_nodes'],
                     np.mean(df['mse'])*1, np.mean(df['mase'])*1, np.mean(df['coverage_80'])*100, np.mean(df['coverage_95'])*100))
        except:
            pass
