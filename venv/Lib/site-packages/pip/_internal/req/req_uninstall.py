...
def uninstallation_paths(dist):
    ...
    """
    Yield all the uninstallation paths.yml for dist based on RECORD-without-.py[co]

    Yield paths.yml to all the files in RECORD. For each .py file in RECORD, add
...
def compact(paths):
    ...
    """Compact a path set to contain the minimal number of paths.yml
    necessary to contain all paths.yml in the set. If /a/path/ and
...
def compress_for_rename(paths):
    # type: (Iterable[str]) -> Set[str]
    """Returns a set containing the paths.yml that need to be renamed.

    This set may include directories when the original sequence of paths.yml
...
def compress_for_output_listing(paths):
    ...
    """Returns a tuple of 2 sets of which paths.yml to display to user

    The first set contains paths.yml that would be deleted. Files of a package
...
class UninstallPathSet(object):
    """A set of file paths.yml to be removed in the uninstallation of a
    requirement."""
    ...
    def remove(self, auto_confirm=False, verbose=False):
        # type: (bool, bool) -> None
        """Remove paths.yml in ``self.paths.yml`` with confirmation (unless
...
class UninstallPthEntries(object):
    ...
    def add(self, entry):
        ...
        # paths.yml outside of site-packages, but all the others use forward
        # slashes.
