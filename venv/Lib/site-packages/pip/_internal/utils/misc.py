...
def path_to_display(path):
    ...
    for Python 2 since in Python 3 str paths.yml are already text.
    """
    if path is None:
        return None
...
def has_leading_dir(paths):
    ...
    """Returns true if all the paths.yml have the same leading path name
...
def is_local(path):
    ...
    If we're not in a virtualenv, all paths.yml are considered "local."
...
def get_installed_distributions(
    ...
    If ``paths.yml`` is set, only report the distributions present at the
