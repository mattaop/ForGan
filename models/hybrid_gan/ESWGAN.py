...
class ESWGAN(GAN):
    ...
    def build_generator(self):
        ...
        historic_shape = (self.window_size,)

        noise_inp = Input(shape=noise_shape)
        historic_inp = Input(shape=historic_shape)

        x = Concatenate(axis=1)([historic_inp, noise_inp])
        x = Dense(16)(x)
    ...
    def build_model(self):
        ...
        time_series = Input(shape=(self.window_size,))
        ...
        print(time_series.shape, forecast.shape)
        valid = frozen_discriminator([time_series, forecast])
    ...
    def fit_gan(self, x, y, epochs=1, batch_size=32, verbose=1):
        ...
        for epoch in range(epochs):
            ...
            forecast_mse[epoch] = mean_squared_error(future_time_series, gen_forecasts)
