...
class DSWGAN(WGAN):
    def __init__(self, cfg):
        ...
        self.plot_folder = 'feed_forward_DSWGAN'
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        coverage_95_PI_2.append(compute_coverage(upper_limits=np.vstack([prediction_mean+1.96*uncertainty]*10000),
                                                 ...
                                                 actual_values=generate_noise(10000)))
    print('80% PI Coverage:', np.mean(coverage_80_PI_1), '+/-', np.std(coverage_80_PI_1))
    print('95% PI Coverage:', np.mean(coverage_95_PI_1), '+/-', np.std(coverage_95_PI_1))

    print('80% PI Coverage:', np.mean(coverage_80_PI_2), '+/-', np.std(coverage_80_PI_2))
    print('95% PI Coverage:', np.mean(coverage_95_PI_2), '+/-', np.std(coverage_95_PI_2))